...
const checks = [
  ...
  ['Store items use one spreadsheet row per Article', ui.includes('store-items-header') && storeCss.includes('grid-template-columns:52px minmax(220px,1.15fr) 110px 110px 110px')],
  ...
  ['Article Master line uses upper free space', storeCss.includes('width:720px') && storeCss.includes('data-store-description') && storeCss.includes('max-width:540px')],
  ['Store lower controls share one aligned height', storeCss.includes('align-items:end!important') && storeCss.includes('height:42px!important') && storeCss.includes('min-height:42px!important')],
