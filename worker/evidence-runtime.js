...
const STORE_IDENTITY_STYLE = '<link rel="stylesheet" href="/store-identity.css?v=complete-identity">';
...
const STORE_IDENTITY_SCRIPT = '<script type="module" src="/store-identity.js?v=complete-identity"></script>';
...
async function withFinalUi(request, response) {
  ...
  if (!html.includes('/store-identity.css')) html = html.replace('</head>', `${STORE_IDENTITY_STYLE}</head>`);
  ...
  if (!html.includes('/store-identity.js')) html = html.replace('</body>', `${STORE_IDENTITY_SCRIPT}</body>`);
